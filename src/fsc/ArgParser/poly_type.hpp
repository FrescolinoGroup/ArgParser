...
namespace detail {
    ...
    struct convert {
        ...
        static
            typename std::enable_if<!std::is_convertible<U, T>::value, T>::type
            from(U const &) {
            throw fsc::O__o("poly_type error: type '" +
...
public:
    ...
    //------------------- cast -------------------
    template <typename T,
              typename enable = std::enable_if_t<drop_casts<T, void>::value>>
    operator T() const {
        if(type() == typeid(int)) {
    ...
    void operator=(
        const char *t) {  // handle spaecial case since "bla" is not a string
        any = std::string(t);
    }
    void operator=(poly_type const &t) { any = t.any; }
    ...
    poly_type &operator+=(char const *t) { return (*this) += std::string(t); }
    ...
    //------------------- const fct -------------------
    std::type_info const &type() const { return any.type(); }
    template <typename S>
